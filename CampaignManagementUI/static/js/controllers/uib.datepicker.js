...
cubeRootApp.controller('datePickerCtrl', ["$scope", "commonService", function ($scope, commonService) {
  ...
  $scope.setStartDate = function () {
	var today = new Date();  
    var startDate = (!!$scope.$parent.startDate) ? $scope.$parent.startDate : new Date(new Date().setDate(today.getDate()-30));
  ...
  $scope.setEndDate = function () {
    var endDate    = (!!$scope.$parent.endDate) ? $scope.$parent.endDate : new Date();
  ...
  if (!!$scope.$parent.dateOptions) {
    $scope.dateOptions = $scope.$parent.dateOptions;
  } else {
    var dt             = new Date();
    $scope.dateOptions = {
      ...
      formatYear : 'yy',
      maxDate    : new Date(),
      minDate    : new Date((dt.getFullYear() - 1) + '-' + (dt.getMonth()) + '-' + dt.getDate()),
      startingDay: 1
    };
  }
  ...
  $scope.init = function () {
    ...
    ($scope.$parent.triggerDateEvent != false) ? $scope.$emit('datesLoaded') : '';
    ...
    setTimeout(function () {
      $scope.$watch('startDate', function () {
        ...
        if ($scope.$parent.startDate !== $scope.startDate) {
          ...
          $scope.$parent.startDate = $scope.startDate;
          $scope.updateCommonService();
          ($scope.$parent.triggerDateEvent != false) ? $scope.$emit('startDateChanged',$scope.startDate) : '';
        }
      }, true);
      ...
      $scope.$watch('endDate', function () {
        if ($scope.$parent.endDate !== $scope.endDate) {
          ...
          $scope.$parent.endDate = $scope.endDate;
          $scope.updateCommonService();
          ($scope.$parent.triggerDateEvent != false) ? $scope.$emit('endDateChanged',$scope.endDate) : '';
        }
      }, true);
    }, 5000)
  };
  ...
  $scope.$on('changeDateRange', function (event, args) {
    ...
    $scope.endDate   = new Date(args.endDate);
  });
}]);